...
/**
 * このプログラムはcheck_differences.shの動作をテストします。
...
// check_differences.shのパス
const checkDiffScript = path.join(workDir, 'check_differences.sh');

// テスト結果を格納する変数
let allTestsPassed = true;
let testResults = [];

// 実行可能かどうか確認し、必要に応じて実行権限を付与
if (!fs.statSync(checkDiffScript).mode & 0o111) {
  console.log('check_differences.shに実行権限を付与します...');
  try {
    fs.chmodSync(checkDiffScript, '755');
  } catch (err) {
    console.error(`実行権限を付与できませんでした: ${err.message}`);
    process.exit(1);
  }
}

/**
 * check_differences.shを実行してテスト
 * @param {string} implFile 実装ファイルパス
 * @param {string} docFile ドキュメントファイルパス
 * @param {boolean} expectedContradiction 矛盾があることを期待するか
 * @returns {object} テスト結果
 */
function runContradictionTest(implFile, docFile, expectedContradiction) {
  console.log(`テスト実行: ${implFile} と ${docFile} の矛盾チェック`);
  console.log(`期待される結果: 矛盾${expectedContradiction ? 'あり' : 'なし'}`);

  try {
    // check_differences.shを実行
    const result = spawnSync(checkDiffScript, [implFile, docFile], {
      encoding: 'utf8',
...
testResults.push(runContradictionTest(correctImplFile, correctDocFile, false));
...
testResults.push(runContradictionTest(incorrectImplFile, incorrectDocFile, true));
